...
from trans_new_text import Trans
import numpy as np
import pickle
import os
...
def test(model,data,test_data,device,tt):
    ''' Start training '''
    relation_nums=len(data['kb']['id2relation'])
    repl_relation=list(range(relation_nums))
    repl_relation_words=[relation2words[m] if m in relation2words else [] for m in repl_relation]
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_data, mininterval=2, desc='  - (Testing)   ', leave=False):
            ...
            score_rel = model(words,words_len,triples_id_rel,triples_sen_rel_padded,triples_len_rel,tt)
...
def main():
    ...
    '''========= Loading Dataset ========='''
    data = pickle.load(open('../dataset_en/fb15k_new/all_data_for_new.pkl','rb'))
    relation2entites=pickle.load(open('../dataset_en/fb15k_new/relation2entity.pkl','rb'))
    ...
    path='checkpoint/2019-07-04_22:39:10'
    ...
    print('test opt',opt)
    transformer.eval()
    mk_uk,hits10_uk,hits3_uk,hits1_uk=test(transformer,data,test_uk, device,1)
    mk_ku,hits10_ku,hits3_ku,hits1_ku=test(transformer,data,test_ku, device,2)
    mk_uu,hits10_uu,hits3_uu,hits1_uu=test(transformer,data,test_uu, device,3)
