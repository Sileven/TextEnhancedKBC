...
class Trans(nn.Module):
    def __init__(
        ...
        self.alpha = torch.nn.Embedding.from_pretrained(torch.empty(n_relation, 3).fill_(0.5).half())
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len,ttt,rel_head,rel_tail):
        """ train:
        ...
        alphas=self.alpha(triple_id[:,1])
        alpha_h = alphas[:, 0:1]
        alpha_r = alphas[:, 1:2]
        alpha_t = alphas[:, 2:3]

        if ttt==1 or ttt==3:
            h = alpha_h*torch.matmul(h_text,self.A)+rel_head
        else:
            h=alpha_h*torch.matmul(h_text,self.A)+(1-alpha_h)*self.entity_bias(triple_id[:,0])
        if ttt==2 or ttt==3:
            t = alpha_t*torch.matmul(t_text,self.B) + rel_tail
        else:
            t=alpha_t*torch.matmul(t_text,self.B)+(1-alpha_t)*self.entity_bias(triple_id[:,2])

        r=alpha_r*torch.matmul(r_text,self.C)+(1-alpha_r)*self.relation_bias(triple_id[:,1])
