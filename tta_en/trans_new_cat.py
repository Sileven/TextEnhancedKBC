...
class Trans(nn.Module):
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len,ttt,rel_head,rel_tail):
        ...
        if ttt==1 or ttt==3:
            h = torch.cat((h_text, rel_head),dim=1)
        else:
            h = torch.cat((h_text, self.entity_bias(triple_id[:,0])),dim=1)
        if ttt==2 or ttt==3:
            t = torch.cat((t_text, rel_tail),dim=1)
        else:
            t = torch.cat((t_text, self.entity_bias(triple_id[:,2])),dim=1)

        r = torch.cat((r_text, self.relation_bias(triple_id[:,1])),dim=1)
