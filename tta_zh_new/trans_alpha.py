# -*- coding: utf-8 -*-
"""
文本和图的结合方式改为 alpha*h_text+(1-alpha)*h_graph
"""
import torch
...
class Trans(nn.Module):
    def __init__(
        ...
        A=torch.Tensor(d_model,d_model)
        torch.nn.init.xavier_uniform_(A)
        self.A = nn.Parameter(A.type(tensor_type))

        B=torch.Tensor(d_model,d_model)
        torch.nn.init.xavier_uniform_(B)
        self.B = nn.Parameter(A.type(tensor_type))

        C = torch.Tensor(d_model, d_model)
        torch.nn.init.xavier_uniform_(C)
        self.C = nn.Parameter(C.type(tensor_type))

        entity_weight=torch.Tensor(n_entity,d_model).uniform_(-6/(d_model**0.5),6/(d_model**0.5))
        relation_weight=torch.Tensor(n_relation,d_model).uniform_(-6/(d_model**0.5),6/(d_model**0.5))

        self.entity_bias=nn.Embedding.from_pretrained(entity_weight,freeze=False)
        self.relation_bias=nn.Embedding.from_pretrained(relation_weight,freeze=False)

        type_convert(self.entity_bias)
        type_convert(self.relation_bias)

        # self.alpha_h=nn.Parameter(torch.tensor(0.5,dtype=tensor_type))
        # self.alpha_r=nn.Parameter(torch.tensor(0.5,dtype=tensor_type))
        # self.alpha_t=nn.Parameter(torch.tensor(0.5,dtype=tensor_type))
        self.alpha_h = nn.Parameter(torch.tensor(0.5).type(tensor_type))
        self.alpha_r = nn.Parameter(torch.tensor(0.5).type(tensor_type))
        self.alpha_t = nn.Parameter(torch.tensor(0.5).type(tensor_type))
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len, enc_text_last, flag, test_flag):
        ...
        h_graph = self.entity_bias(triple_id[:,0])
        t_graph = self.entity_bias(triple_id[:,2])
        r_graph = self.relation_bias(triple_id[:,1])

        h=self.alpha_h*h_text+(1-self.alpha_h)*h_graph
        t=self.alpha_t*t_text+(1-self.alpha_t)*t_graph
        r=self.alpha_r*r_text+(1-self.alpha_r)*r_graph
