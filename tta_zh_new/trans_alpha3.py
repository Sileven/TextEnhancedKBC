# -*- coding: utf-8 -*-
"""
文本和图的结合方式改为loss相加 alpha*h_text*A+(1-alpha)*h_graph 不同的relation对应不同的alpha
"""
import torch
...
class Trans(nn.Module):
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len, enc_text_last, flag, test_flag):
        ...
        h=alpha_h*torch.matmul(h_text,self.A)+(1-alpha_h)*h_graph
        t=alpha_t*torch.matmul(t_text,self.B)+(1-alpha_t)*t_graph
        r=alpha_r*torch.matmul(r_text,self.C)+(1-alpha_r)*r_graph
