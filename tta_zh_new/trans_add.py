...
class Trans(nn.Module):
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len, enc_text_last, flag, test_flag):
        ...
        h = self.entity_bias(triple_id[:,0])
        t = self.entity_bias(triple_id[:,2])
        r = self.relation_bias(triple_id[:,1])

        score_text=torch.sum(torch.abs(h_text + r_text - t_text), 1)
        score_graph=torch.sum(torch.abs(h+r-t),1)

        return score_text+score_graph,enc_te
