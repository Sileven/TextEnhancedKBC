...
class Trans(nn.Module):
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len,ttt,rel_head,rel_tail):
        ...
        if ttt==1 or ttt==3:
            h = alpha_h*h_text+rel_head
        else:
            h=alpha_h*h_text+(1-alpha_h)*self.entity_bias(triple_id[:,0])
        if ttt==2 or ttt==3:
            t = alpha_t*t_text + rel_tail
        else:
            t=alpha_t*t_text+(1-alpha_t)*self.entity_bias(triple_id[:,2])

        r=alpha_r*r_text+(1-alpha_r)*self.relation_bias(triple_id[:,1])
