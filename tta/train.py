...
from trans_alpha3 import Trans
import numpy as np
import pickle
import os
import random
...
def train(model, training_data, optimizer, device, opt, path, start, scheduler):
    ...
    for epoch_i in range(start,opt.epoch):
        ...
        for batch in tqdm(
            ...
            triples_neg_id, triples_neg_sen, triples_neg_sen_len= map(lambda x: x.to(device), batch)
            ...
            loss=nn.functional.margin_ranking_loss(loss_pos,loss_neg,y, margin=opt.margin,reduction='sum')
            #print(loss)
            # gpu_tracker.track()
            loss.backward()
            # update parameters
...
def main():
    ...
    global entity2type
    global type2entities
    global entity2words
    entity2type = data['kb']['entity_id2type_id']
    type2entities = data['kb']['type_id2entity_id']
...
def paired_collate_fn(insts):
    words,triples_pos_id,triples_pos_words,triples_pos_len,types=list(zip(*insts))
    # 文本pad
    ...
    for tri, typ, tri_words, l in zip(triples_pos_id, types, triples_pos_words, triples_pos_len):
        # 正样例
        ...
        if ran_now == 0:
            neg = tri[0]
            while neg == tri[0]:
                neg = random.sample(type2entities[typ[0]], 1)[0]
            ...
            neg = tri[2]
            while neg == tri[2]:
                neg = random.sample(type2entities[typ[1]], 1)[0]
            neg_words=entity2words[neg]
