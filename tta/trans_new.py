...
class Trans(nn.Module):
    ...
    def forward(self, text, text_len, triple_id, triple_words, triple_len,ttt,rel_head,rel_tail):
        ...
        if ttt==1 or ttt==3:
            h = h_text.matmul(self.A)+rel_head
        else:
            h = h_text.matmul(self.A)+self.entity_bias(triple_id[:,0])
        if ttt==2 or ttt==3:
            t = t_text.matmul(self.B) + rel_tail
        else:
            t = t_text.matmul(self.B)+self.entity_bias(triple_id[:,2])

        r = r_text.matmul(self.C)+self.relation_bias(triple_id[:,1])
